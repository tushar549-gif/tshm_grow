...
bot.start(async (ctx) => {
  ...
  // Check total number of users in the database
  ...
  if (!username) {
    return ctx.reply(
      "⚠️ You need to set a username in Telegram before registering.\n\n" +
      "Go to Telegram settings → Username → Set a unique username.\n" +
      "Then, type /start again to register."
    );
  }

  // Get referral UID (if any)
  const referralUID = ctx.message.text.split(" ")[1] || null;
  ...
  user = new User({
    ...
    balance: 0,
    referralBalance: 0,
    referredBy: referralUID ? Number(referralUID) : null,
...
bot.hears("✅ Check-in", async (ctx) => {
  if (!ctx.from || !ctx.from.id) {
    return ctx.reply(
      "⚠️ An error occurred while fetching your ID. Please restart the bot."
    );
  }
  ...
  try {
    ...
    const lastCheckIn = user.lastCheckIn
      ? moment(user.lastCheckIn).startOf("day")
      : null;
...
bot.hears(/^starter pack$/i, (ctx) => {
  const uid = ctx.from?.id;
  if (!uid || !userStates[uid] || userStates[uid].step !== "waiting_for_plan")
    return;

  userStates[uid].step = "waiting_for_proceed";

  ctx.reply(
    "💰 You need to deposit ₹390 to continue.\n🔗 Type *Proceed* to get the payment link. ✅",
    { parse_mode: "Markdown" }
  );
});
...
bot.hears("🔄 Re-activate Plan", async (ctx) => {
  ...
  userStates[uid] = {
    step: "waiting_for_reactivate_plan",
    type: "reactivation",
  };
...
bot.hears(/^starter pack re-activate$/i, (ctx) => {
  ...
  if (
    !userStates[uid] ||
    userStates[uid].step !== "waiting_for_reactivate_plan" ||
    userStates[uid].type !== "reactivation"
  )
    return;
  ...
  ctx.reply(
    "💰 You need to deposit ₹150 to re-activate your plan.\n\n🔗 Type *Re-Activate* to get the payment link. ✅",
    { parse_mode: "Markdown" }
  );
});
...
bot.hears(/^re-activate$/i, async (ctx) => {
  ...
  if (
    !uid ||
    !userStates[uid] ||
    userStates[uid].step !== "waiting_for_reactivate"
  )
    return;
...
bot.hears("📂 Deposit History", async (ctx) => {
  ...
  try {
    ...
    deposits.forEach((deposit) => {
      ...
      historyMessage += `💰 *Amount:* ₹${
        deposit.amount
      }\n📅 *Date:* ${formattedDate}\n✅ *Status:* ${
        deposit.status.charAt(0).toUpperCase() + deposit.status.slice(1)
      }\n🔄 *Type:* ${
        deposit.isReactivation ? "Re-activating" : "Purchasing"
      }\n-------------------------\n`;
    });
...
bot.hears("💰 Payout", async (ctx) => {
  ...
  try {
    ...
    if (day < 2 || day > 25) {
      return ctx.reply(
        "❌ Withdrawals are allowed only from the 2nd to 25th of each month."
      );
    }
    ...
    if (lastWithdrawal && new Date(lastWithdrawal.date).getDate() === day) {
      return ctx.reply(
        "❌ You can only withdraw once per day. Try again tomorrow!"
      );
    }

    if (withdrawalsThisMonth.length >= 2) {
      return ctx.reply(
        "❌ You have reached the limit of 2 withdrawals per month."
      );
    }

    if (user.status !== "active") {
      return ctx.reply(
        "❌ Your account is inactive. Purchase a plan to activate withdrawals!"
      );
    }
    ...
    if (withdrawalsThisMonth.length === 0) {
      ...
      return ctx.reply(
        "You are eligible to withdraw ₹400. Enter your UPI ID 🔢💳."
      );
    } else {
...
bot.hears(/^[A-Za-z ]+$/, async (ctx) => {
  ...
  try {
    ...
    return ctx.reply(
      "🎉 Withdrawal Successful! Your request will be processed within 24-72 hours."
    );
  } catch (error) {
    console.error("Error saving withdrawal:", error);
    ctx.reply(
      "❌ An error occurred while processing your withdrawal. Try again later."
    );
  }
});
...
bot.hears("📊 Balance", async (ctx) => {
  try {
    ...
    ctx.reply(
      "⚠️ An error occurred while fetching your balance. Please try again later."
    );
  }
});
...
bot.hears("👥 Affiliate", async (ctx) => {
  try {
    ...
    ctx.reply(
      "⚠️ An error occurred while fetching your affiliate data. Please try again later."
    );
  }
});
...
bot.action("move_to_balance", async (ctx) => {
  ...
  try {
    ...
    if (user.referralBalance < 100) {
      return ctx.answerCbQuery(
        "❌ You need at least ₹100 in referral earnings to move to balance!",
        { show_alert: true }
      );
    }
    ...
    return ctx.answerCbQuery(
      "✅ ₹100 has been moved to your balance successfully!",
      { show_alert: true }
    );
  } catch (error) {
    console.error("Error processing move to balance:", error);
    ctx.answerCbQuery(
      "❌ An error occurred while moving to balance. Try again later.",
      { show_alert: true }
    );
  }
});
...
bot.hears("📌 Profile", async (ctx) => {
  try {
    ...
    ctx.reply(
      "⚠️ An error occurred while fetching your profile. Please try again later."
    );
  }
});
